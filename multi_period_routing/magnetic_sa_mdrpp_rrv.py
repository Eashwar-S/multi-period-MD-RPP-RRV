...
# ══════════════════════════════════════════════════════════════════════════════
# §3  Multi-trip planner for ONE vehicle (multiple battery charges)
# ══════════════════════════════════════════════════════════════════════════════
...
def plan_vehicle_trips(G: nx.Graph, dist: dict, depot: int,
    ...
    or all target_edges are covered.

    Uses trip_cache keyed by (depot, frozenset(target_canonical)) to avoid recomputation.

    Returns
    -------
    trips        : list of node-sequence lists
    total_travel : sum of trip travel times
    covered      : set of canonical edges covered
    mission_time : total_travel + (n_trips - 1) * recharge_time
    """
    ...
    result = (trips, total_travel, covered_total, mission_time, trip_times)
    # trip_cache[cache_key] = result
    return result


# ══════════════════════════════════════════════════════════════════════════════
# §4  Objective function (matches Gurobi formulation)
# ══════════════════════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════════════════════
# §5  Initial edge allocation (greedy probability-weighted assignment)
# ══════════════════════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════════════════════
# §7  Full solution builder (all vehicles)
# ══════════════════════════════════════════════════════════════════════════════
...
def simulated_annealing_magnetic_no_cache(G: nx.Graph,
                                   dist: dict,
                                   depots: List[int],
                                   edge_probs: Dict[Tuple, float],
                                   capacity: float,
...
def run_one_graph(meta: dict, graph_dir: str,
    ...
    trip_cache = global_cache.setdefault(graph_id, {})

    best_trips, best_obj, best_prob, best_beta, all_obj = simulated_annealing_magnetic_no_cache(
