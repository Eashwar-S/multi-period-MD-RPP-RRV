...
MEMORY_CONFIG: dict = dict(
    ...
    min_direct_fit=0.60,    # fraction of best possible fit to accept without repair
    min_repair_fit=0.50,    # fraction of best possible fit to attempt repair
    max_memory_size=2000,   # evict oldest if exceeded
)

# Experiment configuration
NUM_INSTANCES   = 1
N_REPEATS       = 5   # independent SA repeats per (mode × instance) for mean±std
RANDOM_SEED_BASE = 42
...
class ProbabilityRankedTripMemory:
    ...
    def retrieve(self,
        ...
        target_set = set(can(*e) for e in target_edges)

        candidates = [
            m for m in self._bank
            if m.graph_id == graph_id and m.vehicle_home_depot == home_depot
            and (m.covered_edges & target_set)  # pre-filter: must overlap targets
        ]
...
def repair_trip(route: List[int],
    ...
    for target_e in sorted_targets:
        ...
        for endpoint in (a, b):
            ...
            if total_needed <= capacity + 1e-9:
                try:
                    if paths and current in paths and endpoint in paths[current]:
                        seg = paths[current][endpoint]
                    else:
    ...
    if current not in depot_set:
        ...
        try:
            if paths and current in paths and nd in paths[current]:
                seg = paths[current][nd]
            else:
...
def plan_vehicle_trips_with_memory(
    ...
    candidates = memory_bank.retrieve(graph_id, depot, target_edges, edge_probs, all_edges)
    ...
    for score, mem in candidates:
        ...
        if current_fit >= cfg['min_repair_fit'] * best_possible_fit:
            ...
            repaired_route, rep_cost, newly_covered = repair_trip(
                ...
                edge_probs, uncovered_target, paths=paths)
...
def make_memory_planner(
    ...
    def planner_fn(depot: int, target_edges: List[Tuple]):
        return plan_vehicle_trips_with_memory(
            ...
            memory_bank, graph_id, instance_id, stats, all_edges, paths=paths, cfg=_cfg)
...
def simulated_annealing_unified(
    ...
    for iter_idx in range(max_iterations):
        ...
        # Log progress periodically for large instances so it doesn't look stuck
        if iter_idx > 0 and iter_idx % 50 == 0:
            log.info(f"    [SA Inner] {mode:<18} | Rep={repeat} Inst={instance_id} | Iter={iter_idx}/{max_iterations} | best_obj={best_obj:.2f} | no_improve={no_improve}")

        if no_improve >= no_improve_limit:
            log.info(f"    [SA Inner] {mode:<18} | Rep={repeat} Inst={instance_id} | Early stop at iter {iter_idx} (no_improve limit)")
            break
...
def generate_probability_instances(
        G: nx.Graph,
        n_instances: int) -> List[Dict[Tuple, float]]:
    """
    ...
    Extracts the probability of edges from the 'prob' key without seed-based variations.
    """
    instances = []
    ...
    for i in range(n_instances):
        instances.append(dict(base_probs))
...
def solve_instance(
    ...
    if mode == 'baseline':
        ...
        planner_fn = make_memory_planner(
            ...
            graph_id, instance_id, all_edges, stats, paths=paths) # Pass paths
...
def run_one_repeat(
        ...
        T_interval: float,
        max_flights: int,
        prob_instances: List[Dict[Tuple, float]],
        graph_id: int,
        seed_base: int,
        sa_kwargs: dict) -> List[ExperimentRecord]:
    """
    Worker function to run all instances for a single (mode, repeat) combination.
    Extracted to the module level so it can be pickled by multiprocessing on Windows.
    """
    ...
    # Pre-compute all-pairs shortest paths (done once, passed to workers)
    # This was moved from run_experiment to here for the new signature
    ...
    for inst_id, edge_probs in enumerate(prob_instances):
        ...
        log.info(f"[Worker] Running: {mode} | Repeat {rep} | Instance {inst_id}")

        rec = solve_instance(
            G, dist, paths, depots, capacity, recharge_time, T_interval, # Pass paths
...
def run_experiment(graph_dir: str,
    ...
    repetitions on the same graph, using concurrent execution.

    Repeat structure
    ----------------
    Each repeat is a completely independent replication of the full
    sequence. Within a repeat:
    ...
    # Pre-compute all-pairs shortest path lengths (done once, passed to workers)
    dist: Dict[int, Dict[int, float]] = dict(
        nx.all_pairs_dijkstra_path_length(G, weight='weight'))

    # ── Generate probability instances ─────────────────────────────────────────
    ...
    for mode in modes:
        for rep in range(n_repeats):
            tasks.append((mode, rep))
            
    log.info('Generated %d instances × %d repeats × %d modes = %d SA runs total.',
             n_instances, n_repeats, len(modes), len(tasks) * n_instances)
    log.info('Submitting %d task chunks to ProcessPoolExecutor...', len(tasks))
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=os.cpu_count()) as executor:
        futures = {
            executor.submit(
                run_one_repeat,
                mode, rep, G, dist, depots, capacity, recharge_time, T_interval,
                max_flights, prob_instances, graph_id, seed_base, sa_kwargs
            ): (mode, rep)
            for mode, rep in tasks
        }
        
        for future in tqdm(concurrent.futures.as_completed(futures), total=len(futures), desc="SA Repeats"):
            mode, rep = futures[future]
            try:
                result_records = future.result()
                records.extend(result_records)
                log.info(f"Chunk finished: mode={mode:<20} rep={rep}")
            except Exception as e:
                log.error(f"Chunk failed: mode={mode} rep={rep} error={e}")
...
if __name__ == '__main__':
    ...
    for graph_id in range(1,16):
        out_dir    = os.path.join(script_dir, 'memory_results_small',f'graph_{graph_id}')
        os.makedirs(out_dir, exist_ok=True)

        log.info('Starting PRTMB experiment on graph %d with %d instances × %d repeats',
                graph_id, NUM_INSTANCES, N_REPEATS)
